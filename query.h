...
void dfs_cycle(int u, int p, int color[], int par[], int &cyclenumber, int cycles[], ofstream &file, Graph graph) {
    ...
    for (int v : graph.g[u]) {
        ...
        dfs_cycle(v, u, color, par, cyclenumber, cycles, file, graph);
    }
...
void fora_query_topk_with_bound(int v, const Graph& graph){
    ...
    const static double lowest_delta_rmax = config.epsilon*sqrt(min_delta/3/graph.m/log(2/new_pfail));//更新后的r_max
...
void fora_query_topk_with_bound2(int v, const Graph& graph){
    Timer timer(0);
    const static double min_delta = 1.0 / graph.n;
    const static double init_delta = 1.0 / 4;
    threshold = (1.0-config.ppr_decay_alpha)/pow(500, config.ppr_decay_alpha) / pow(graph.n, 1-config.ppr_decay_alpha);//?
...
void get_topk2(int v, Graph &graph){
    ...
    const static double lowest_delta_rmax = config.epsilon*sqrt(min_delta/3/graph.m/log(1/new_pfail));//更新后的r_max
    const static double lowest_delta_rmax_2 = min_epsilon*sqrt(graph.m * 1.0 * config2.delta / graph.n / 3 / log(1 / new_pfail));
    ...
    for (int j = 0; j < graph.n; ++j) {
        ...
        int degree_all=0;
        for (int nei:graph.g[j]) {
            degree_all+=graph.g[nei].size();
        }
        upper_bounds_self.insert(j,config.alpha+(1-config.alpha)*(1-config.alpha)*config.alpha*degree_re/degree_all+
                                   (1-config.alpha)*(1-config.alpha)*(1-config.alpha)*config.alpha*(1-degree_re*1.0/degree_all));
        upper_bounds_self_init.insert(j,upper_bounds_self[j]);
    }
    bool fora_flag=true,stop;
    dht_candidate.reset_one_values();
    ...
    do{
        ...
        int candidate_num=0;
        stop=if_stop2(fora_flag,raw_epsilon,candidate_num);
        //选择算法
        bool f=true,b=true;
        ...
        if (f==true&& b==true){
            ...
            double rmax_new=config.epsilon * sqrt(delta_new / 3 / graph.m / log(2 / config.pfail))*config.rmax_scale;
            assert(rsum!=0);
            double omega=rsum*(2 + config.epsilon) * log(2 / config.pfail) / delta_new / config.epsilon / config.epsilon;
            double cost_fora=omega+1/rmax_new-1/config.rmax;
            double epsilon_new=max(min_epsilon, config2.epsilon / 2);
            rmax_new= epsilon_new * sqrt(graph.m * 1.0 * config2.delta / 3.0 / graph.n / log(2.0 / config2.pfail)) * config2.rmax_scale;
            omega= rmax_new * 3 * log(2.0 / config2.pfail) / config2.delta / epsilon_new / epsilon_new;
            double cost_bippr=omega+1/rmax_new-1/config.rmax;
